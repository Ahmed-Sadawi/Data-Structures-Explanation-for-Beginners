...
An **array** is a collection of elements, each identified by an index or a key, stored in contiguous memory locations. All elements in an array are of the same data type.
...
- **Random Access:** Accessing any element by its index is fast and takes constant time, O(1).
